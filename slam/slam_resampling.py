...
def resampling(particles):
    ...
    Edited by Philip Tasche to use a naive resampling technique
    """
    ...
    for i in range(N_PARTICLE):
        ...
        particles[i].yaw = tmp_particles[j].yaw
        particles[i].lm = tmp_particles[j].lm[:, :]
        particles[i].lmP = tmp_particles[j].lmP[:, :]
        particles[i].w = 1.0 / N_PARTICLE
...
def main():
    ...
    particles = [Particle(n_landmark, np.array([0, 0, 0])) for _ in range(N_PARTICLE)]
