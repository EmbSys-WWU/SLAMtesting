...
float metric(int *xs1, int *ys1, int *xs2, int *ys2, size_t size1, size_t size2)
{
    float metric_value = -1.0;
    
    // Resource allocation
    feature_t   *m1 =   (feature_t*) malloc(size1 * sizeof(feature_t));
    feature_t   *m2 =   (feature_t*) malloc(size2 * sizeof(feature_t));
    float       *w1 =       (float*) malloc(size1 * sizeof(float));
    float       *w2 =       (float*) malloc(size2 * sizeof(float));
    signature_t *s1 = (signature_t*) malloc(sizeof(signature_t));
    signature_t *s2 = (signature_t*) malloc(sizeof(signature_t));
    
    if (m1 && m2 && w1 && w2 && s1 && s2)
    {
        // Value initialization
        int i;
        for (i = 0; i < size1; i++)
        {
            w1[i]   = 1.0;   // All have the same weight
            m1[i].x = xs1[i];
            m1[i].y = ys1[i];
        }
        for (i = 0; i < size2; i++)
        {
            w2[i]   = 1.0;   // All have the same weight
            m2[i].x = xs2[i];
            m2[i].y = ys2[i];
        }
        s1->n        = size1;
        ...
        // Call to EMD function
        metric_value = emd(s1, s2, euclidean_distance, NULL, NULL);
...
static void russel(double *S, double *D)
{
  ...
  for(i=0; i < _n1 ; i++)
    for(j=0; j < _n2 ; j++)
      Delta[i + j * (_n1 + 1)] = _C[i][j] - Ur[i].val - Vr[j].val;
	  ...
	  for (CurV=vHead.Next; CurV != NULL; CurV=CurV->Next)
	    {
	      int j;
	      j = CurV->i;
	      if (deltaMin > Delta[i + j * (_n1 + 1)])
		{
		  deltaMin = Delta[i + j * (_n1 + 1)];
		  minI = i;
		  minJ = j;
		  PrevUMinI = PrevU;
		  PrevVMinJ = PrevV;
